...
class BilinearSimilarity(SimilarityFunction):
    """This similarity function performs a bilinear transformation of the two
    input tensors. This function has a matrix of weights W and a bias b, and
    the similarity between two vectors x and y is computed as `x^T W y + b`.
    """
    ...
    def initialize_weights(self,
        ...
        self.weight_matrix = K.variable(
            self.initializer((tensor_1_dim, tensor_2_dim)),
            name=self.name + "_weights")
        self.bias = K.variable(self.initializer(1,), name=self.name + "_bias")
